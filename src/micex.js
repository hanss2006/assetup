...
class Micex {
  ...
  static getSecurityInfo(security) {
    ...
    return Micex.securityDefinition(security)
      .then((data) => {
        ...
        if (!boards.length)
          throw new Error(`Security ${security} doesn't have any board in definition`);
        let board = boards[0];
        let info = {
          engine: board.engine,
          market: board.market
  ...
  static _responseToSecurities(response, requestParams) {
    ...
    let securitiesInfoIndex = _.keyBy(blocks.securities, indexBy);
  ...
  static securityDefinition(security = required('security')) {
    return Micex._request(`securities/${security}`)
      .then((response) => {
        ...
        security.description = _.keyBy(security.description, 'name');
        security.boards = _.keyBy(security.boards, 'boardid');
        return security;
      });
  }
